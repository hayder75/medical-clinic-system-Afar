            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border p-6 lg:col-span-2">
                <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                  ...
                  {selectedTechnician ? `Tests by: ${selectedTechnician.name}` : 'Lab Technician Revenue List'}
                </h3>

                {selectedTechnician ? (
                  <div>
                    <button
                      onClick={() => { setSelectedTechnician(null); setCurrentPage(1); }}
                      className="mb-4 flex items-center gap-1 text-sm text-indigo-600 hover:text-indigo-800"
                    >
                      <ChevronLeft className="h-4 w-4" /> Back to all technicians
                    </button>
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        ...
                        <tbody>
                          {(reportData.tests || [])
                            .filter(t => t.processedBy === selectedTechnician.name)
                            .slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE)
                            .map((test, idx) => (
                              <tr key={idx} className="border-t border-gray-100 hover:bg-gray-50">
                                <td className="px-4 py-3 text-sm">{new Date(test.createdAt).toLocaleDateString()}</td>
                                <td className="px-4 py-3 text-sm font-medium">{test.patientName || 'N/A'}</td>
                                <td className="px-4 py-3 text-sm">{test.testName}</td>
                                <td className="px-4 py-3 text-sm"><span className="px-2 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs">{test.testCategory}</span></td>
                                <td className="px-4 py-3">
                                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${test.status === 'COMPLETED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{test.status}</span>
                                </td>
                              </tr>
                            ))}
                        </tbody>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b text-left text-xs text-gray-500 uppercase">
                          ...
                          <th className="py-2 pr-2">Total Tests</th>
                      ...
                      <tbody>
                        {reportData.byTechnician?.length > 0 ? (
                          reportData.byTechnician.map((tech, idx) => (
                            <tr
                              key={tech.name}
                              onClick={() => { setSelectedTechnician(tech); setCurrentPage(1); }}
                              className="border-b last:border-b-0 cursor-pointer hover:bg-indigo-50 transition-colors"
                            >
                              <td className="py-3 pr-2 text-sm font-medium text-gray-900">{tech.name}</td>
                              <td className="py-3 pr-2 text-sm font-bold text-indigo-600">{tech.total}</td>
                              <td className="py-3 pr-2 text-sm text-green-600">{tech.completed}</td>
                              <td className="py-3 pr-2 text-sm text-yellow-600">{tech.pending}</td>
                              <td className="py-3 pr-2">
                                <div className="flex items-center gap-2">
                                  <div className="w-16 h-2 bg-gray-200 rounded-full">
                                    <div className="h-2 bg-green-500 rounded-full" style={{ width: `${tech.total > 0 ? (tech.completed / tech.total) * 100 : 0}%` }}></div>
                                  </div>
                                  <span className="text-xs text-gray-600">{tech.total > 0 ? Math.round((tech.completed / tech.total) * 100) : 0}%</span>
                                </div>
                              </td>
                              <td className="py-3 pr-2 text-sm font-semibold text-emerald-700">{formatCurrency(tech.revenue || 0)}</td>
                            </tr>
                          ))
                        ) : (
                          <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-500">No technician data for this period</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
