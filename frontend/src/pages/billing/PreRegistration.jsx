...
const PreRegistration = () => {
  ...
  const [patientSearchType, setPatientSearchType] = useState('name'); // name, phone, or id
  const [selectedPatient, setSelectedPatient] = useState(null);

  const searchPatientApi = useCallback(async (query, signal) => {
    const response = await api.get(`/pre-registration/search-patients?query=${query}&type=${patientSearchType}`, { signal });
                ...
                <div className="space-y-2">
                  <div className="flex space-x-2">
                    <select
                      ...
                      onChange={(e) => {
                        setPatientSearchType(e.target.value);
                        setPatientSearchQuery('');
                        setPatientSearchResults([]);
                      }}
                  ...
                  <input
                    ref={searchInputRef}
                    type="text"
                        placeholder={`Search by ${patientSearchType === 'name' ? 'name' : patientSearchType === 'phone' ? 'phone number' : 'patient ID'}...`}
                  ...
                  {selectedPatient && (
                    <div className="p-2 bg-green-50 border border-green-200 rounded-lg">
                      <div className="text-sm font-medium text-green-800">Selected: {selectedPatient.name}</div>
                      <div className="text-xs text-green-600">
                        {selectedPatient.mobile && `Phone: ${selectedPatient.mobile}`}
                        {selectedPatient.mobile && selectedPatient.id && ' • '}
                        {selectedPatient.id && `ID: ${selectedPatient.id}`}
                      </div>
                      <button
                        onClick={() => {
                          setSelectedPatient(null);
                          setAddForm({...addForm, patientId: null});
                        }}
